...
exports.run = function(payload, parsed) {
    ...
    async.series([
        function(mainCallback) {
            log.info('Check if YUIDoc was already installed');
            if (!file.exists('node_modules/yuidocjs')) {
                yuidoc.install(mainCallback);
            } else {
                mainCallback();
            }
        },
        function(mainCallback) {
            log.info('Run YUIDoc and watches for any changes');
