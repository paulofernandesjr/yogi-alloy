...
exports._checkBranch = function(mainCallback) {
    prompt('What branch do you want to deploy [' + deployBranch + ']? ', function(ans) {
        if (ans.trim() !== '') {
            deployBranch = ans;
        }
        mainCallback();
...
exports._checkRemote = function(mainCallback) {
    prompt('What remote do you want to deploy [' + deployRemote + ']? ', function(ans) {
        if (ans.trim() !== '') {
            deployRemote = ans;
        }
        mainCallback();
...
exports._gitCommit = function(mainCallback) {
    command.open(root)
        .on('stdout', command.writeTo(process.stdout))
        .exec(which('git'), ['commit', '-m', 'Regenerate'])
        .then(function() {
            mainCallback();
        });
};
...
exports._gitPushToBranch = function(mainCallback, remote, branch) {
    command.open(root)
        .on('stdout', command.writeTo(process.stdout))
        .exec(which('git'), ['push', remote, branch])
        .then(function() {
            mainCallback();
        });
};
...
exports._gitGoToBranch = function(mainCallback, branch) {
    command.open(root)
        .on('stdout', command.writeTo(process.stdout))
        .exec(which('git'), ['checkout', branch])
        .then(function() {
            mainCallback();
        });
};
