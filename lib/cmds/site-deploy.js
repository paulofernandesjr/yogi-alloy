...
exports._moveOutFolder = function(mainCallback, payload) {
    ...
    async.eachLimit(outCollection, 10, function (filename, callback) {
        ...
        // If user send a parameter, then it should deploy to that specific website version
        if (typeof payload !== "string") {
            destination = root + '/' + filename;
        } else {
            destination = root + '/versions/' + payload + '/' + filename;
        }
...
exports._goBackToCurrentBranch = function(mainCallback, payload) {
    var currentBranch = '';

    // If user send a parameter, then it should goes back to that specific branch
    if (typeof payload !== "string") {
