...
exports.COMMAND = {
    ...
    description: 'Build current folder component or walk dirs trying to build all found components. Try `--js` or `--css` with or without `--fast` flag.'
};
...
exports.buildCSS = function(mainCallback) {
    ...
    command.open(alloyBootstrapDir)
        ...
        .exec(which('yogi'), ['alloy', '--compile-css' ])
        .then(function() {
            ...
            command.open(auiCssCssDir)
                .on('stdout', command.writeTo(process.stdout))
                .exec(which('yogi'), ['alloy', '--namespace-css', alloyJSON.cssnamespace ])
                .then(function() {
                    mainCallback();
                });
        });
