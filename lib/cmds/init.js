...
exports.buildYUI = function(srcDir, buildDir, message, version, walk, fast, mainCallback) {
    var _build = function() {
        ...
        command.open(srcDir)
            ...
            .exec(which('shifter'), args, { cwd: srcDir })
            .then(function() {
                mainCallback();
            });
    };
...
exports._initDependency = function(dependency, depCallback) {
    ...
    command.open(root)
        .on('stdout', command.writeTo(process.stdout))
        .exec(which('git'), ['clone', dependency.repo, '-b', dependency.version, dependency.folder], { cwd: root })
        .then(function() {
            depCallback();
        });
};
...
exports._initNPM = function(mainCallback) {
    ...
    prompt('Do you wish to initialize npm modules [y/N] ', function(ans) {
        if (ans.toLowerCase() === 'y') {
            command.open(root)
                .on('stdout', command.writeTo(process.stdout))
                .exec(which('npm'), ['install'], { cwd: root })
                .then(function() {
                    mainCallback();
                });
        }
    ...
    async.series([
        function(mainCallback) {
            command.open(root)
                ...
                .exec(which('git'), ['fetch', dependency.repo, dependency.version], { cwd: dependency.folder })
                .then(function() {
                    mainCallback();
                });
        },
        function(mainCallback) {
            command.open(root)
                .on('stdout', command.writeTo(process.stdout))
                .exec(which('git'), ['checkout', dependency.version], { cwd: dependency.folder })
                .then(function() {
                    mainCallback();
                });
        },
        function(mainCallback) {
            command.open(root)
                .on('stdout', command.writeTo(process.stdout))
                .exec(which('git'), ['pull', '--rebase', dependency.repo, dependency.version], { cwd: dependency.folder })
                .then(function() {
                    mainCallback();
                });
        }],
